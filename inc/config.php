 ...
 */
	...
	// custom filters, applied by ghoost
	 $config['filters'][] = array(
	 	'condition' => array(
	 		'body' => '/test$/i',
	 		'body' => '/Test$/i',
	 	...
	 	'action' => 'reject',
	 	'message' => 'Post detected; Flood discarded.'
	 );
 ...
 */
	...
	// Ditto, but for the name field.
	$config['field_name_selectbox'] = false;

	// Catalog button(s).
	$config['catalog_button'] = true;
 ...
 */
	...
	$config['anonymous'] = array('Chud', 'Cafe Chai Soylent', 'Cafe Mocha Soylent', 'Banana Soylent', 'Creamy Chocolate Soylent', 'Mint Chocolate Soylent', 'Original Soylent', 'Strawberry Soylent', 'Vanilla Soylent', 'Cacao Soylent (Soytan)', 'Soyak');
 ...
 */
	...
	$config['post_date'] = '%A, %B %e, %Y, %H:%M:%S';
	// Same as above, but used for "you are banned' pages.
	$config['ban_date'] = '%A, %B %e, %Y';
	...
	$config['footer'][] = _('All trademarks, copyrights, comments, and images on this page are owned by and are the responsibility of their respective parties.<br><br>Email to <a href="mailto:leghoost@atomicmail.io">leghoost@atomicmail.io</a> for any questions about the software, and anything else.<br><br>Credits to <a href="mailto:yoot55@proton.me">yoot</a> for some of the code.');
	...
	$config['stylesheets']['Wows Dark'] = 'wows-dark.css';
	$config['stylesheets']['Keyed Frog'] = 'keyed_frog.css';
	...
	/*
	 * For lack of a better name, “boardlinks” (or "boardlist(s)" are those sets of navigational links that appear at the top
 ...
 */
	...
	$config['error']['dnsbl']		= _('TOR IP detected; visit %s for more information.');
 ...
 */
	...
	$config['custom_capcode']['Mod'] = array(
		...
		'color:purple;font-weight:bold', // Change name style; optional
		'color:purple' // Change tripcode style; optional
	);

    $config['custom_capcode']['Developer'] = array(
	...
	$config['custom_capcode']['Owner'] = array(
    		'<span class="capcode" style="color:darkgreen;font-weight:bold"> ## %s</span> <img src="/static/stretched_owner_badge" alt="This user is an owner." title="This user is an owner." style="width:18px;height:18px;vertical-align:middle;margin-right:6px">',
 ...
 */
	...
	$config['mod']['themes'] = DEVELOPER;
	$config['mod']['rawhtml'] = DEVELOPER;
	$config['mod']['rebuild'] = DEVELOPER;
	...
	$config['mod']['move'] = JANITOR; // move it janny
 ...
 */
	...
	/*$config['mod']['custom_pages']['/something(?:\/(\d+))?'] = function($id = null) {
	global $mod;
	mod_page('Something', 'mod/something.html', $mod);

	 	if (!hasPermission($config['mod']['rules']))
	 		error($config['error']['noaccess']);
	 	// ...
	};*/
	
	// Overboard but for mods.
	require_once("templates/themes/ukko/theme.php");
	
	$config['mod']['custom_pages']['/overboard(?:\/(\d+))?'] = function($id = null) {
	        global $mod;
	
	        $ukko = new ukko();
	        $ukko->settings = array();
	        $ukko->settings['uri'] = 'overboard';
	        $ukko->settings['title'] = 'Mod Overboard';
	...
	$config['board_regex'] = '[0-9a-zA-Z$_\x{0080}-\x{FFFF}]{1,64}';
	...
	// Use the turnstile CAPTCHA for reports?
	$config['report_turnstile_captcha'] = false;
