	...
	switch ($version) {
		...
		case '4.4.97':
			if (!isset($_GET['confirm2'])) {
				$page['title'] = 'License Change';
				$page['body'] = '<p style="text-align:center">You are upgrading to a version which uses an amended license. The licenses included with somechan distributions prior to this version (4.4.98) are still valid for those versions, but no longer apply to this and newer versions.</p>' .
					'<textarea style="width:700px;height:370px;margin:auto;display:block;background:white;color:black" disabled>' . htmlentities(file_get_contents('LICENSE.md')) . '</textarea>
					<p style="text-align:center">
		...
		case '4.5.2':
			if (!isset($_GET['confirm3'])) {
				...
				$page['body'] = '<p style="text-align:center">You are upgrading to the 5.0 branch of somechan. Please back up your database, because the process is irreversible. At the current time, if you want a very stable somechan experience, please use the 4.5 branch. This warning will be lifted as soon as we all agree that 5.0 branch is stable enough</p>
					<p style="text-align:center">
		...
		case false:
			// TODO: enhance Tinyboard -> somechan upgrade path.
		...
		default:
			...
			$page['body'] = '<p style="text-align:center">somechan was unable to determine what version is currently installed.</p>';
			break;
		case VERSION:
			$page['title'] = 'Already installed';
			$page['body'] = '<p style="text-align:center">It appears that somechan is already installed (' . $version . ') and there is nothing to upgrade! Delete <strong>' . $config['has_installed'] . '</strong> to reinstall, but if you used git; use the <strong>git pull</strong> command, and other stuff.</p>';
			break;
	}			
...
if ($step == 0) {
	...
	$tests = array(
		array(
			...
			'required' => true,
			'message' => 'somechan requires PHP 5.4 or better.',
		),
		array(
			'category' => 'PHP',
			'name' => 'PHP &ge; 5.6',
			...
			'required' => false,
			'message' => 'somechan works best on PHP 5.6 or better.',
		),
		array(
			'category' => 'PHP',
		...
		array(
			...
			'prereq' => '',
			'result' => $can_exec && shell_exec('echo "somechan" | md5sum') == "141225c362da02b5c359c45b665168de  -\n",
			'required' => false,
		...
		array(
			...
			'result' => $can_exec && shell_exec('echo "somechan" | /sbin/md5 -r') == "141225c362da02b5c359c45b665168de\n",
			'required' => false,
		...
		array(
			...
			'result' => is_writable('.'),
			'required' => true,
			'message' => 'somechan does not have permission to create directories (boards) here. You will need to <code>chmod</code> (or operating system equivalent) appropriately.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => true,
			'message' => 'You must give somechan permission to create (and write to) the <code>templates/cache</code> directory or performance will be drastically reduced.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => true,
			'message' => 'You must give somechan permission to write to the <code>tmp/cache</code> directory.'
		),
		array(
			'category' => 'File permissions',
			...
			'required' => false,
			'message' => 'somechan does not have permission to make changes to <code>inc/secrets.php</code>. To complete the installation, you will be asked to manually copy and paste code into the file instead.'
		),
		array(
			'category' => 'Misc',
		...
		array(
			'category' => 'Misc',
			'name' => 'somechan installed using git',
			'result' => is_dir('.git'),
			'required' => false,
			'message' => 'The .git directory in your root files were not detected. Please use git to make upgrading somechan easier'
		)
	);
...
';
	...
	$queries[] = Element('posts.sql', array('board' => 'bgr'));
	$queries[] = Element('posts.sql', array('board' => 'news'));
	...
	$page['body'] = '<center>Thank you for using somechan. Please remember to report any bugs you discover. <a href="https://github.com/vichan-devel/vichan/wiki/Configuration-Basics">Please read this if you want to learn to modify the config files.</center></a>';
	...
	if (!empty($sql_errors)) {
		$page['body'] .= '<div class="ban"><h2>SQL errors</h2><p>SQL errors were encountered when trying to install the database. This may be the result of using a database which is already occupied with a somechan installation; if so, you can probably ignore this.</p><p>The errors encountered were:</p><ul>' . $sql_errors . '</ul><p><a href="?step=5">Ignore errors and complete installation.</a></p></div>';
	} else {
		$boards = listBoards();
		foreach ($boards as &$_board) {
			setupBoard($_board);
	...
	$page['title'] = 'Installation complete';
	$page['body'] = '<p style="text-align:center">Thank you for using somechan. Please remember to report any bugs you discover.</p>';
